...
conf.calDir = fullfile(getPath(source),'test1');
conf.dataDir = fullfile(getPath(source),'processed','testing_phow');
...
conf.vocabPath = fullfile(getPath(source),'processed','kaggle_phow','baseline-vocab.mat');
...
ims = dir(fullfile(conf.calDir, '*.jpg'))' ;
ims = vl_colsubset(ims, conf.numTrain + conf.numTest) ;
ims = cellfun(@(x)fullfile(x),{ims.name},'UniformOutput',false) ;
images = {images{:}, ims{:}} ;
