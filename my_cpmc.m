function cpmc_example(img_name,exp_dir,savePath)  
%    addpath('./code/');    
...
%    exp_dir = './data/';
%    exp_dir = '/afs/ee.cooper.edu/user/t/a/tam8/AFSdrop/CATSnDOGS/processed/data/';
...
% Save if path given
if nargin>2
    nSave = min([10 numel(scores)]);
    save_masks = masks(:,:,1:nSave);
    save_scores = scores(1:nSave);
    save(strcat(savePath,img_name,'.mat'),'save_masks','save_scores')
